...
:- module(dcpf,[step_particle_aux/5,logIndepOptimalProposals/3,indepOptimalProposals/3,optimalproposal/7,likelihood_weighting/3,normalize/2,printp22/1,printp/1,printp/0,set_magic/1,step_particle3/1,step_particle/3,step_particle/4,step_particle/5,step_particle_ps/2,step_particle_ps/3,step_particle_ps/4,eval_query_particle/3,eval_query_particle2/4,eval_query_particle3/5,init_particle/1,init_particle/2,init_particle/3]).
...
printp22(Pos) :-
	dcpf:bb_get(offset,Offset),
	I is Offset+Pos,
	%obsProbability(object(3))
	findall((I,D),(recorded(I,current(wanted_action(human(1))) ~= D,_), writeln(I),writeln(D)),_).
...
logweight_constraints(Iteration,I,O,C,P) :-
	...
	logweight_backward(I,O,W) ->
		true;
		( write(logweight_backward(I,O,W)),writeln(' failed') )
	),
	test_constraints(I,C,PC),!,
	%writeln(eval_weight_backward(I,O,W)),
	(
...
eval_query_particle3(T,X,Q,N,P) :-
	...
	findall((I2,List),part(T,I2,List),List2),
	%writeln(List2),
	!.
